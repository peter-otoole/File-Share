...
module.exports = {
	...
	getRoot:function(request,response){
		...
		log.info("Returning base/index.html page.");
		response.sendfile("./views/base/index.html");
	},
	
	getOutOfSessionHome: function(request,response){
	
		var log = logger("view-handlers.getOutOfSessionHome");
		
		log.info("Returning extras/out-of-session-home.html page");
		response.sendfile("./views/extras/out-of-session-home.html");
	},
	
	getSignInPage: function(request, response){
	
		var log = logger("view-handlers.getSignInPage");
		
		log.info("Returning Sign In Page.");
		response.sendfile("./views/extras/sign-in.html");
	},
	
	getBasicServerData: function(request, response){
		
		var log = logger("view-handlers.getBasicServerData");
		
		log.info("Entered getBasicServerData()");
		dbc.getBasicServerData(function(err, res, message){
			if(err){
				log.error("An internal server error has occurred. Here is the message: "+message);
				response.send(common.INTERNAL_SERVER_ERROR, {code: common.INTERNAL_SERVER_ERROR, message: common.INTERNAL_SERVER_ERROR_MSG});
			}else{
				if(res.code === 200){
					log.info("Got basic data, returning to user.");
					response.send(common.OK, {code: common.OK, serverName: res.name, message: common.OK_MSG});
				}else{
					log.info("Could not find basic data in the database, returning code: "+common.BAD_REQUEST);
					response.send(common.BAD_REQUEST, {code: common.BAD_REQUEST, message: common.BAD_REQUEST_MSG});
				}
			}
		});
	},
	
	getConfigurationPage: function(request, response){
	
		var log = logger("view-handlers.getConfigurationPage");
		
		log.info("Returning Configuration Page.");
		response.sendfile("./views/extras/configuration.html");
	},
		...
		dbc.signIn(email, password, function(err, res, message){
			...
			if(err){
				...
				if( res.code === 200 ){
					...
					response.send(common.OK, {code: common.OK, message: common.OK_MSG, name: res.name, session: res.session});
				}else{
					log.info("Cannot sign user in, returning "+common.NOT_FOUND);
					response.send(common.NOT_FOUND, {code: common.NOT_FOUND, message: common.NOT_FOUND_MSG});
				}
			}
		});
	},
		...
		dbc.logOut(token, function(err,res,message){
			...
			if(err){
				...
				if( res.code === 200 ){
					...
					response.send(common.OK, {code: common.OK, message: common.OK_MSG+" Signed out."});
				}else{
					log.info("Could not signed user out, returning "+common.NOT_FOUND);
					response.send(common.NOT_FOUND, {code: common.NOT_FOUND, message: common.NOT_FOUND_MSG+"Could not sign out."});
				}
			}
		});
	},
		...
		dbc.createUser(email, fName, lName, key, password, function(err, res, message){
			...
			if( err ){
				...
				if(res.code === 200){
					...
					response.send(common.OK, {code: common.OK, message: common.OK_MSG});
				}else if(res.code === 403){
					log.info("Could not create user, key is invalid. Returning "+common.FORBIDDEN);
					response.send(common.FORBIDDEN, {code: common.FORBIDDEN, message: common.FORBIDDEN_MSG});
				}else{
				...
				if( res.code === 200 ){
					...
					dbc.getWhiteList(function(error, result, mess){
						...
						if( error || result.list == [] ){
							...
							response.send(common.OK, {code: common.OK, message: common.OK_MSG, list: result.list});
						}
					});
				}else{
					log.info("User session cannot be validated, returning "+common.REAUTHENTICATE);
					response.send(common.REAUTHENTICATE, {code: common.REAUTHENTICATE,message: common.REAUTHENTICATE_MSG});
				}
			}
		});
	},
