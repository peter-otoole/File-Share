		...
		mongo.connect(common.DB_URL_COMPLETE, function (err, db) {
			...
			try{
				...
				if (err) {
					setUp = false;
					log.error("An error occurred while attempting to connection to the database. Error: "+err, 4);
					callback(true, setUp, "Could not connect to the database! Error: "+err);
				} else {
					log.info("Successfully connected to the database.");
					
					log.info("Tables exist: "+db.getCollectionNames().length == 4);
							
					if( db.getCollectionNames() ==4 ){
						setUp = true;
				...
				callback(true, setUp, "Could not connect to the database! Error: "+e);
			}
		});
	},
